  ...
  return (
    <div className="group relative bg-white rounded-[40px] overflow-hidden border-4 border-[#3A332F] hover:shadow-[12px_12px_0px_0px_#C14B3A] transition-all duration-500 hover:-translate-y-2 flex flex-col h-full">
      ...
      <button 
        ...
        className={`absolute top-8 right-8 z-30 p-3 bg-white rounded-full transition-all shadow-xl border-2 active:scale-75 hover:scale-110 ${
          isFavorite 
      ...
      >
        ...
        <div className="flex items-center justify-between pt-4 border-t-2 border-[#FDF5E6] mt-auto">
          <span className="text-2xl font-ghibli-title text-[#3A332F]">
            <span className="text-[#C14B3A] text-base mr-1">$</span>{product.price.toFixed(2)}
          </span>
          <button 
            onClick={(e) => { 
              e.stopPropagation(); 
