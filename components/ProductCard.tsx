  ...
  return (
    ...
    >
      ...
      <div className="px-3 pb-3 md:px-8 md:pb-8 md:pt-2 space-y-2 md:space-y-3 flex-grow flex flex-col">
        ...
        <h3 className="font-ghibli-title md:font-ghibli-title text-lg md:text-xl text-[#C14B3A] line-clamp-2 leading-tight h-12 md:h-14">
          {product.name}
        </h3>

        <div className="flex items-center justify-between pt-2 md:pt-4 border-t md:border-t-2 border-[#FDF5E6] mt-auto">
          <span className="text-xl md:text-xl font-ghibli-title md:font-ghibli-title text-[#3A332F]">
            <span className="text-[#C14B3A] mr-1 text-sm md:text-xl">$</span>{formatCurrency(product.price)}
          </span>
          <button
            onClick={(e) => {
              e.stopPropagation();
